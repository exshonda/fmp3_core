...
/*
 *		シリアルインタフェースドライバのチップ依存部（IMX8MM用）
 *		（非TECS版専用）
 */
...
sio_opn_por(ID siopid, EXINF exinf)
{
	 ...
	 *  デバイス依存のオープン処理
	 */
	p_siopcb = imx8uart_opn_por(siopid, exinf);
...
{
	return(imx8uart_snd_chr(p_siopcb, c));
}
...
{
	return(imx8uart_rcv_chr(p_siopcb));
}
...
{
	imx8uart_ena_cbr(p_siopcb, cbrtn);
}
...
{
	imx8uart_dis_cbr(p_siopcb, cbrtn);
}
