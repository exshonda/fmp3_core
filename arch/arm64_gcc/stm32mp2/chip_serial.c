...
/*
 *		シリアルインタフェースドライバのチップ依存部（STM32MP2用）
 *		（非TECS版専用）
 */
...
sio_opn_por(ID siopid, EXINF exinf)
{
	 ...
	 *  デバイス依存のオープン処理
	 */
	p_siopcb = stm32usart_opn_por(siopid, exinf);
...
{
	return(stm32usart_snd_chr(p_siopcb, c));
}
...
{
	return(stm32usart_rcv_chr(p_siopcb));
}
...
{
	stm32usart_ena_cbr(p_siopcb, cbrtn);
}
...
{
	stm32usart_dis_cbr(p_siopcb, cbrtn);
}
 ...
 */
void
stm32usart_irdy_snd(EXINF exinf)
{
	sio_irdy_snd(exinf);
}
 ...
 */
void
stm32usart_irdy_rcv(EXINF exinf)
{
	sio_irdy_rcv(exinf);
}
