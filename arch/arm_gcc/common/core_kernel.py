...
#
#  riscv_gcc/common/core_kernel.py・arm64_gcc/common/core_kernel.py と同じ
#  ガード．chip_kernel.py／target_kernel.py 等が設定する想定．Ruby版では
#  未代入の $グローバル変数参照はnil（偽）として扱われるが，Pythonでは
#  未定義グローバル参照はNameErrorになるためここでガードする．
#
...
#    dict として供給する）．
#
#  原文の `0.upto($TMAX_INHNO).each_with_index do |localInhnoVal, index|` は，
#  Rubyの0.upto(N)が既に0起点の連番であるためlocalInhnoValとindexが常に
#  等しい恒等変換になる．range(TMAX_INHNO + 1)1本に単純化して翻訳した．
#
