...
#   の責任を負わない．
#
#  $Id: plic_kernel.py (converted from plic_kernel.trb by Claude Code Sonnet 5) $
#

#
#  パス2の生成スクリプトのコア依存部（PLIC用）
#

#
#  PLIC割込みターゲットコンテキストINDEXテーブル
#
kernelCfgC.comment_header("PLIC Interrupt target Context Index Table")
kernelCfgC.add("const uint8_t _kernel_plic_target_cidx_table[PLIC_TNUM_INTNO + 1] = {")
for index, intno in enumerate(plic_intno_list):
    if index > 0:
        kernelCfgC.add(",")
