...
`DEF_MPK` が定義するカーネルメモリプールは，dcre 忠実の**単純な bump
アロケータ**である（TLSF 等の汎用アロケータではない）。`malloc_mpk`/
`free_mpk`（`kernel/kernel_impl.h`）経由で使う。

- 確保は「現在のブレークポイントから必要バイト数を切り出して前進させる」
  だけの単純な操作。**断片化管理を一切行わない**。
- **返却された確保数（`count`）がゼロになったときだけ，ブレークポイントを
  プール先頭へ戻す**。つまり「途中の1個だけ返す」操作ではメモリは戻らず，
  「全部返し切った」ときにだけ先頭から再利用できるようになる。この特性
  ゆえに，`del_*` → `acre_*` を繰り返す典型的な使い方では，同じ領域が
  周期的に再利用される（`test_dcre4` の反復確保テストが実演している）。
- ギャント・ロック下でしか呼ばれないため，アロケータ自体は排他制御を
  持たない。
...
**注意（未 hardening）**：`acre_dtq`/`acre_pdq`/`acre_mpf` は，管理領域
サイズ（`sizeof(DTQMB) * dtqcnt` 等）やブロック領域サイズ
（`ROUND_MPF_T(blksz) * blkcnt`）を計算する際の**乗算オーバーフローを
検査しない**（dcre 由来，本移植でも hardening していない）。`size_t` が
桁あふれすると，本来収まらないはずの確保が「成功」してしまう経路が理論上
存在する。§7・§9 も参照。
...
いずれも「giant lock の外でアプリケーションが直接触れる資源」と「bump
アロケータの再利用」が絡む，理論上の競合ウィンドウである。実害が
到達可能であることをテストで実証したものではないが，発生条件を明示した
上で受容する判断をしている。
...
`TSTAT_DORMANT` を観測して `del_tsk` → `free_mpk` でき，プールの全確保数
が0になれば（bump アロケータの再利用条件），次の `acre_tsk` が同一メモリを
...
→ タスクBが `del_mpf`（プールの確保数が0になり bump の先頭がリセット）
→ タスクBが別種の `acre_*`（新しい管理領域が同一バイト列に確保される）
...
### 7.5 サイズ乗算オーバーフロー未検査（hardening 予定）

§6 末尾のとおり，`acre_dtq`/`acre_pdq`/`acre_mpf` はサイズ計算の乗算
オーバーフローを検査しない。`kernel/startup.c` の `malloc_mempool` が持つ
符号混在比較のバグ（`(ptrdiff_t) >= (size_t)` の比較。§9 上流報告候補 c）
と組み合わさると，ユーザが巨大な `dtqcnt`/`pdqcnt`/`blkcnt` を渡す誤用経路
で悪化しうる。段階1の deferred #1（§7.2）・段階3aの `acre_mtx` 未検査
`ceilpri` と同系統の「ユーザ誤用経路の hardening」課題として引き継がれて
おり，**到達可能性の実証も修正も行っていない**。
