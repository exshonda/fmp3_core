...
機能コード（TFN_ACRE_SEM 等6個）は kernel_fncode.h に既存の見込み — 実装前確認（§8-1）。

### 1.3 エラーと検査（dcre 準拠）

- `acre_sem`: CHECK_VALIDATR(sematr, TA_TPRI)、`isemcnt <= maxsem`・`1 <= maxsem <= TMAX_MAXSEM`
  の範囲検査（dcre semaphore.c の acre_sem と同一式）、E_NOID。
...
- `del_*` 共通: E_NOEXS（TA_NOEXS）→ E_OBJ（静的 = id <= tmax_s*id）→ 成功。
...
- **del_mtx はロック中でも成功**（dcre mutex.c:430-475 と同一）:
  `p_loctsk != NULL` なら `remove_mutex(p_loctsk, p_mtxcb)` で所有タスクのチェーンから
  外し、ceiling mutex なら優先度復帰を行う。FMP3 の `remove_mutex`（mutex.c:223-236）と
...
- 待ち解除で起きたタスクの再スケジュールは init_wait_queue / make_non_wait が面倒を見る
  （既存機構）。del_* 側でディスパッチ判断が必要かは ini_* の実装を正とする（§8-3）。

## 3. cfg 層

- `kernel_api.def` に dcre と同一の3行: `AID_SEM .nosem` / `AID_FLG .noflg` / `AID_MTX .nomtx`。
- **per-object テンプレート（semaphore.trb/.py 等）の変更はゼロ**（全て汎用 KernelObject
  継承であることを現物確認済み。dcre 自身も per-object テンプレートを持たない）。
- 訂正E ガード（AID>0 かつ静的0個 → cfg エラー）が3オブジェクトにも自動適用される。
  各オブジェクトで発火することのエラー回帰 cfg を追加（`dcre_aid_sem_no_static.cfg` 等、
  in-class E_RSATR 回帰と合わせて計6ケース）。
...
- 2レンジ ID マクロ: SEMID/FLGID/MTXID を段階2の CYCID と同型で新規定義または置換
  （現行実装の有無・形は実装前確認 §8-2。CYCID の p_*inib ポインタ判定・(ID) キャスト・
  `tnum_* - tnum_s*` 境界の形を踏襲）。
...
3. 混在 AID（AID_CYC>0 / AID_ALM=0 等）の equivalence サンプル cfg（同 Minor 3）。
   段階3a の AID 追加で組合せが増えるため、sem/flg/mtx の混在も1ケースに含める。
4. DIVERGENCE_MAP の cyclic.c 行へ CHECK_PAR 恒真条件除去の半文追記（同 Minor 4）。
...
6. 訂正E ガードが sem/flg/mtx で発火することの事前確認（段階2で cyclic により実証済みだが
   オブジェクト種依存の穴がないこと）
