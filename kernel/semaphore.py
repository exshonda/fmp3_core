...
#   の責任を負わない．
# 
#  $Id: semaphore.py (converted from semaphore.trb by Claude Code Sonnet 4.6) $
# 
...
class SemaphoreObject(KernelObject):
    ...
    def prepare(self, key, params):
        # sematrが無効の場合（E_RSATR）
        if (params["sematr"] & ~(TA_TPRI)) != 0:
            error_illegal_id("E_RSATR", params, "sematr", "semid")

        # maxsemが有効範囲外の場合（E_PAR）
        if not (1 <= params["maxsem"] and params["maxsem"] <= TMAX_MAXSEM):
            error_illegal_id("E_PAR", params, "maxsem", "semid")

        # isemcntが有効範囲外の場合（E_PAR）
    ...
    def generateInib(self, key, params):
        return (f"({params['sematr']}), ({params['isemcnt']}), "
                f"({params['maxsem']})")
