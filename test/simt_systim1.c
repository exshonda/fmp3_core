 ...
 * 
 *  Copyright (C) 2014-2025 by Embedded and Real-Time Systems Laboratory
 ...
 *  の責任を負わない．
 * 
 *  $Id: simt_systim1.c 1214 2026-05-26 08:18:29Z ertl-hiro $
 */
 ...
 *	63:	DO(simtim_advance(20U))
 *		assert(fch_hrt() == 1120U)							// 時刻：1120
 *	64:	sta_alm(ALM2, 100U)									// 発生：1221
 *	65:	chg_ipm(TIPM_ENAALL)
 *	== HRT_HANDLER											// 時刻：1120
 *	== ALM1-6（6回目）==
 *	66:	assert(fch_hrt() == 1130U)							// 時刻：1130
 *	67:	DO(simtim_advance(10U))
 *		assert(fch_hrt() == 1140U)							// 時刻：1140
 *	// 高分解能タイマ割込みを強制的に入れる
 *	68:	DO(raise_hrt_int())
 *		RETURN
 *	69:		[hook_hrt_set_event <- 81U]
 *	== HRT_HANDLER						... (C-2)			// 時刻：1140
 *	// スプリアス割込み										// 時刻：1150
 *	70:		[hook_hrt_set_event <- 71U]
 *	== TASK1（続き）==
 *	71:	DO(simtim_advance(10U))
 *	72:	assert(fch_hrt() == 1160U)							// 時刻：1160
 *	73:	slp_tsk()
 *	== HRT_HANDLER											// 時刻：1221
 *	== ALM2-5（5回目）==
 *	74:	assert(fch_hrt() == 1231U)							// 時刻：1231
 *	75:	wup_tsk(TASK1)
 *		RETURN
 ...
 *	// スプリアス割込み										// 時刻：1241
 *	78:		[hook_hrt_set_event <- HRTCNT_EMPTY]
 *	== TASK1（続き）==
 *	79:	DO(simtim_advance(59U))
 *		assert(fch_hrt() == 1300U)							// 時刻：1300
 *	// タイムイベントを登録後に時間を進め，先頭のタイムイベントの発生時
 *	// 刻が過ぎた状況を作る
 *	80:	sta_alm(ALM1, 100U)									// 発生：1401
 *	81:		[hook_hrt_set_event <- 101U]
 *	82:	adj_tim(200)					... (A-2)			// ALM1の発生：1201
 *	83:		[hook_hrt_raise_event]
 *	== HRT_HANDLER											// 時刻：1300
 *	== ALM1-7（7回目）==
 *	84:	assert(fch_hrt() == 1310U)							// 時刻：1310
 *		RETURN
 *	85:		[hook_hrt_set_event <- HRTCNT_EMPTY]
 *	== TASK1（続き）==
 *	// タイムイベントを登録後に時間を戻し，先頭のタイムイベントの発生時
 *	// 刻までの高分解タイマのカウントアップ値がHRTCNT_BOUNDを超える状況
 *	// を作る
 *	86:	sta_alm(ALM1, TMAX_RELTIM)					// 発生：4,000,001,311
 *	87:		[hook_hrt_set_event <- 4000000001U]
 *	88:	adj_tim(-200)					... (A-4)	// ALM1の発生：4,000,001,511
 *	89:		[hook_hrt_set_event <- HRTCNT_BOUND]
 *	90:	slp_tsk()
 *	== HRT_HANDLER									// 時刻：4,000,001,312
 *	// スプリアス割込み								// 時刻：4,000,001,322
 *	91:		[hook_hrt_set_event <- 189U]
 *	== HRT_HANDLER									// 時刻：4,000,001,511
 *	== ALM1-8（8回目）==
 *	92:	assert(fch_hrt() == 4000001521U)			// 時刻：4,000,001,521
 *	93:	wup_tsk(TASK1)
 *		RETURN
 *	94:		[hook_hrt_set_event <- HRTCNT_EMPTY]
 *	== TASK1（続き）==
 *	95:	END
 */
...
extern void _kernel_simtim_target_raise_hrt_int(void);
extern void _kernel_simtim_target_raise_hrt_int(void);
...
{
	if (sns_loc()) {
		_kernel_simtim_target_raise_hrt_int();
	}
	else {
		loc_cpu();
		_kernel_simtim_target_raise_hrt_int();
