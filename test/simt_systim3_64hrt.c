 ...
 *  の責任を負わない．
 * 
 *  $Id: simt_systim3_64hrt.c 1214 2026-05-26 08:18:29Z ertl-hiro $
 */
 ...
 *	24:	chg_ipm(TMIN_INTPRI)
 *	25:	DO(simtim_advance(15U))
 *		assert(fch_hrt() == 1535U)							// 時刻：1535
 *	26:	ref_cyc(CYC2, &rcyc)			... (C-1)	// CYC2の発生時刻を
 *		assert(rcyc.lefttim == 0U)					//			過ぎている
 *	27:	chg_ipm(TIPM_ENAALL)
 *	== HRT_HANDLER											// 時刻：1535
 *	== CYC2-3（3回目）==							// CYC2の次回発生：2030
 *	28:	assert(fch_hrt() == 1545U)							// 時刻：1545
 *		RETURN
 *	29:		[hook_hrt_set_event <- 475U]
 *	== TASK1（続き）==
 *	30:	DO(simtim_advance(15U))
 *	31:	assert(fch_hrt() == 1560U)							// 時刻：1560
 *	// ここで時間が経過したことを想定
 *	32:	DO(simtim_advance(40U))
 *		assert(fch_hrt() == 1600U)							// 時刻：1600
 *	33:	sta_alm(ALM1, 100U)								// ALM1の発生：1701
 *	34:		[hook_hrt_set_event <- 101U]
 *	35:	sta_alm(ALM2, 200U)								// ALM2の発生：1801
 *	// ここで時間が経過したことを想定
 *	36:	DO(simtim_advance(101U))
 *	== HRT_HANDLER											// 時刻：1701
 *	== ALM1-1（1回目）==
 *	37:	assert(fch_hrt() == 1711U)							// 時刻：1711
 *	38:	stp_alm(ALM2)					... (B-1)
 *		RETURN
 *	39:		[hook_hrt_set_event <- 309U]
 *	== TASK1（続き）==
 *	40:	END
 */
